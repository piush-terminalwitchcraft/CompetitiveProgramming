...
void solve() {
    ...
    ll sm = 0;
    rep(i,0,n) if(i%2) sm-=a[i]; else sm += a[i];
    if(sm==0){
        cout<<"1\n1 "<<n<<"\n";
    }
    else{
        vector<pair<ll,pll>> b; ll len = 0;
        rep(i,0,n){
            if(a[i]!=0) b.pb({a[i],{i-len,i}}),len=0;
            else len++; 
        }
        deb(b);
        vpll ans;
        for(int i = 0; i < sz(b); i++){
                if(i+1<n){  
                    if(b[i].ff==b[i+1].ff) 
                    ans.pb({b[i].ss.ff,b[i+1].ss.ss});
                    else 
                    ans.pb({b[i].ss.ff,b[i].ss.ss}),ans.pb({b[i+1].ss.ff,b[i+1].ss.ss});
                    i++;
                }
                else{
                    cout<<"-1\n"; 
