...
void solve() {
    ...
    // else{
        vll b = a;reverse(all(b));
        if(is_sorted(all(b)))cout<<a[0]+1;
        else{
            ll diff=0;
            for(int i = 1; i < n; i++){
                if(a[i]<a[i-1]){
                    ll sm = a[i-1]+a[i], tmpdiff = (sm+1)/2;
                    if(abs(a[i]-sm/2) >= abs(a[i-1]-sm/2)) tmpdiff = sm/2;
                    diff = max(diff,tmpdiff);
                }
            }
            for(int i = 0; i < n; i++)a[i]=abs(a[i]-diff);
            if(is_sorted(all(a)))cout<<diff;
            else cout<<-1;
        }
    // }
                    // deb(" ");
